...
## 0. 1.13.0 What's New（App Store 更新说明草稿）

> 基于 `CHANGELOG.md` 的 `1.13.0` 整理。提交 ASC 前请再核对本次发版范围是否全部包含。

```
- 新增首次打开时的轻量问卷，让 Arya猫了解你的名字、职业和使用目标。
- 新增猫猫人设设置：可自定义猫猫名字、说话风格、陪伴角色、主动性和记忆偏好。
- 新增「关于我」和「猫猫人设」设置入口，之后也能随时调整。
- 猫猫记忆升级为长期画像、互动偏好、近期状态和近期计划，记忆更贴近当下，也更容易管理。
- 情绪梳理练习会在对话中自然引导，不再切到割裂的独立模式。
- 修复周回顾偶发只有思考气泡、没有实际总结的问题，并优化 Trace Dashboard 的稳定性。
```

精简版（如果想更短）：

```
- 新增首次打开问卷，让 Arya猫更懂你的背景和目标。
- 新增猫猫人设与「关于我」设置，可自定义名字、风格、陪伴方式和记忆偏好。
- 猫猫记忆升级，支持更清晰地管理长期信息、互动偏好和近期状态。
- 情绪梳理练习改为在对话中自然引导，上下文更连贯。
- 修复周回顾和 Trace Dashboard 的若干稳定性问题。
```
...
**Q：能导出我的数据吗？**
A：当前版本暂未提供独立导出入口。数据默认保存在本机；你也可以手动开启 iCloud 同步，让同一 Apple ID 的设备同步记录。
